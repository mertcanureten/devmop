...
import BoxComponent from "ink-box"; // For decorative boxes
import htm from "htm";
...
const Header = ({ totalSize }) => html`
  ...
  >
    <Box>
      <Text bold color="cyan">🧹 devmop </Text>
      <Text color="gray">| Terminal Dashboard</Text>
    </Box>
    <Box>
      <Text>Total potentially recoverable: </Text>
      <Text bold color="yellow">${prettyBytes(totalSize)}</Text>
    </Box>
  </Box>
`;

const Sidebar = ({ plugins, onSelect, selectedPlugin }) => {
  ...
  return html`
    <Box 
      ...
      flexDirection="column"
    >
      <Text bold underline marginBottom="${1}">Plugins</Text>
      <${SelectInput} 
        items="${items}" 
        onSelect="${onSelect}"
        itemComponent="${({ label, isSelected, item }) => html`
          <Box>
            <Text color="${isSelected ? "cyan" : "white"}">
              ${isSelected ? "❯ " : "  "}${label.padEnd(10)}
            </Text>
            <Text color="gray"> [${prettyBytes(item.size)}]</Text>
          </Box>
        `}"
      />
    </Box>
...
const MainArea = ({ plugin, onClean, isCleaning }) => {
  ...
  const barWidth = 30;
  const percentage = 100; // Simplified for TUI
  ...
  return html`
    ...
    >
      <Box marginBottom="${1}">
        <Text bold color="cyan">${plugin.name.toUpperCase()} </Text>
        <Text color="gray">— ${plugin.description}</Text>
      </Box>

      <Box flexDirection="column" marginBottom="${1}">
        <Text>Estimated Size: <Text bold color="yellow">${prettyBytes(plugin.size || 0)}</Text></Text>
        <Text color="cyan">${bar}</Text>
      </Box>

      <Box flexGrow="${1}" flexDirection="column">
        <Text color="gray" italic>Paths to scan:</Text>
        <Box flexDirection="column" marginTop="${1}">
          <Text>• Caches and temporary logs</Text>
          <Text>• Build artifacts (if applicable)</Text>
        </Box>
      </Box>
...
const Footer = () => html`
  ...
  >
    <Box marginRight="${2}">
      <Text bold color="gray">KEYS: </Text>
    </Box>
    <Box marginRight="${2}">
      <Text color="cyan">↑/↓</Text> <Text color="gray">Navigate</Text>
    </Box>
    <Box marginRight="${2}">
      <Text color="green">C</Text> <Text color="gray">Clean Selected</Text>
    </Box>
    <Box marginRight="${2}">
      <Text color="yellow">A</Text> <Text color="gray">Clean All</Text>
    </Box>
    <Spacer />
    <Box>
      <Text color="red">Q</Text> <Text color="gray">Quit</Text>
    </Box>
  </Box>
`;
...
const App = () => {
  ...
  useInput(async (input, key) => {
    ...
    if (input === "c" && !isCleaning) {
      ...
      if (plugin) {
        setIsCleaning(true);
        await plugin.clean({ dry: false });
        // Refresh size
        const newSize = await plugin.getSize();
