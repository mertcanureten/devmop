...
export async function sendNotification(title, message) {
  try {
    ...
    if (IS_WINDOWS) {
      ...
      const ps = [
        ...
        "[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('DevMop').Show($toast)",
      ].join("; ");
  ...
  console.log(chalk.bgRed.white.bold(` 🔔 DevMop: ${message} `));
}
    ...
    if (total >= thresholdBytes) {
      if (!lastAlerted) {
        ...
        await sendNotification(
          "DevMop — Disk Alert 🚨",
          `${totalHuman} recoverable — run \`devmop clean --all\``
        );
        lastAlerted = true;
